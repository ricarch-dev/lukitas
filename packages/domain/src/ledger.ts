...
export function createLedgerFact(input: LedgerFact): LedgerFact {
  ...
  if (!Number.isFinite(Date.parse(input.occurredAt)))
    throw new RangeError('occurredAt must be a valid instant');
  if (input.amount._decimal.coefficient <= 0n)
    throw new RangeError('Ledger amounts must be positive');
  if (input.kind === 'OPENING' && input.amount.currency.precision < 0)
...
export function balanceFor(
  ...
  return facts.reduce(
    (balance, fact) => balance.add(signedEffect(fact.kind, fact.amount)),
    openingBalance,
  );
}
